...
class UpdatePyqMcq extends StatefulWidget {
  final String docId;
  ...
  @override
  _UpdatePyqMcqState createState() => _UpdatePyqMcqState();
}

class _UpdatePyqMcqState extends State<UpdatePyqMcq> {
  ...
  Future<String?> _uploadImage(
    ...
    try {
      final fileName =
          "${widget.docId}_${label}_${widget.questionId}_${DateTime.now().millisecondsSinceEpoch}${p.extension(imageFile.path)}";
      final storageRef =
          FirebaseStorage.instance.ref().child('pyq_images/$fileName');
  ...
  Future<void> _updateMCQ() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("PYQ MCQ updated successfully!"),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Error updating PYQ MCQ: $e"),
