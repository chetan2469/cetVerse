...
class TestSideBar extends StatelessWidget {
  ...
  Map<String, int> _getSubjectAttempted() {
    ...
    for (var i = 0; i < mcqs.length; i++) {
      String subject = mcqs[i]['subject'] ?? 'Unknown';
      if (userAnswers[i] != -1) {
  ...
  Map<String, int> _getSubjectTotal() {
    ...
    for (var mcq in mcqs) {
      String subject = mcq['subject'] ?? 'Unknown';
  ...
  Widget build(BuildContext context) {
    ...
    final isPcm = pyqType.toLowerCase() == 'pcm';
    final subjects = ['Physics', 'Chemistry', isPcm ? 'Maths' : 'Biology'];
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ...subjects.map((subject) {
                      final att = subjectAttempted[subject] ?? 0;
                      final tot = subjectTotal[subject] ?? 0;
                      return Padding(
